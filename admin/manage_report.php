...
try {
    $query = "SELECT id, name, email, address, issue_type, description, pdf_file, TRIM(video_file) AS video_file, status, created_at FROM reports WHERE 1=1";
    $params = [];
        ...
        if (!$report_id || !$name || !$email || !$issue_type || !$description || !$status) {
            ...
            if (isset($_FILES['pdf_upload']) && $_FILES['pdf_upload']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!is_dir($upload_dir)) {
                    ...
                    } elseif (!chmod($upload_dir, 0777)) {
                        $error = "Failed to set permissions on uploads directory.";
                    }
                }
            ...
            if (!$error && isset($_FILES['video_upload']) && $_FILES['video_upload']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!$error) {
                    if (!in_array($file_type, $allowed_types)) {
                        $error = "Only MP4 or WebM videos are allowed.";
                    } elseif ($file_size > $max_size) {
                            ...
                            <div class="report-card-body">
                                ...
                                <?php if ($report['video_file']): ?>
                                    <p class="report-card-text">
                                        ...
                                        <?php 
                                        $upload_dir = 'C:/xampp/htdocs/takecare/uploads/';
                                        $video_path = $upload_dir . $report['video_file'];
                                        $video_url = '/takecare/uploads/' . htmlspecialchars($report['video_file']);
                                        ?>
                                        <?php if (file_exists($video_path)): ?>
                                            <video controls width="100px" height="auto">
                                                <source src="<?php echo $video_url; ?>" type="video/<?php echo strtolower(pathinfo($report['video_file'], PATHINFO_EXTENSION)); ?>">
                                                Your browser does not support the video tag. <a href="<?php echo $video_url; ?>" target="_blank">Download video</a>
                                            </video>
                                            <!-- Debugging output -->
                                            <small class="text-muted">File path: <?php echo htmlspecialchars($video_path); ?></small>
                                        <?php else: ?>
                                            <span class="text-danger">Video file not found: <?php echo htmlspecialchars($report['video_file']); ?> (Path: <?php echo htmlspecialchars($video_path); ?>)</span>
                                        <?php endif; ?>
                                    </p>
                                <?php endif; ?>
                                ...
                                <div class="modal-body">
                                    <form method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="file" class="form-control" id="video_upload<?php echo $report['id']; ?>" name="video_upload" accept="video/mp4,video/webm">
                                            <?php if ($report['video_file']): ?>
